...
import { generateCatalogBrief } from '@/app/briefs/generateCatalogBrief';
...
export async function POST(req: Request) {
  ...
  const catalogId = String(formData.get('catalogId') ?? DEFAULT_CATALOG_PARTNER_ID);
  const partner = catalogPartnerById(catalogId);
  if (!partner) {
  ...
  const result = await generateCatalogBrief({
    sourceText,
    projectTitle,
  ...
  const { data: saved, error } = await admin
    ...
    .insert({
      ...
      mode: brief.mode,
      target: brief.classification || partner.name,
  ...
  if (error || !saved) {
    console.error('Save catalog brief error:', error);
